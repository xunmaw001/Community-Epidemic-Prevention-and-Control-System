    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑隔离监控</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑隔离监控</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">隔离监控信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="gongzuoId" name="gongzuoId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>隔离开始时间</label>
                                        <input id="geliStartTime-input" name="geliStartTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>隔离结束时间</label>
                                        <input id="geliEndTime-input" name="geliEndTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>隔离地点</label>
                                        <input id="geliAddress" name="geliAddress" class="form-control"
                                               placeholder="隔离地点">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>身体状况</label>
                                    ...
                                    <div class="form-group  col-md-6">
                                        <label>备注</label>
                                        <input id="geliContentupload" name="file" type="file">
                                        <script id="geliContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var geliContentUe = UE.getEditor('geliContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="geliContent" id="geliContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#geliContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('geliContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("geli/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addgeli');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.geliContent != null && ruleForm.geliContent != 'null' && ruleForm.geliContent != '' && $("#geliContentupload").length>0) {

            var geliContentUeditor = UE.getEditor('geliContentEditor');
            geliContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.geliContent != null){
                    mes = ''+ ruleForm.geliContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                geliContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#geliContentupload").length>0) {
            var geliContentEditor = UE.getEditor('geliContentEditor');
            if (geliContentEditor.hasContents()) {
                $('#geliContent-input').attr('value', geliContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                zhuhuId: "required",
                gongzuoId: "required",
                geliStartTime: "required",
                geliEndTime: "required",
                geliAddress: "required",
                shentiTypes: "required",
                geliContent: "required",
            },
            messages: {
                zhuhuId: "住户不能为空",
                gongzuoId: "工作人员不能为空",
                geliStartTime: "隔离开始时间不能为空",
                geliEndTime: "隔离结束时间不能为空",
                geliAddress: "隔离地点不能为空",
    ...
    // 获取当前详情
    function getDetails() {
        var addgeli = window.sessionStorage.getItem("addgeli");
        if (addgeli != null && addgeli != "" && addgeli != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("geli/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function dateTimePick() {
            laydate.render({
                elem: '#geliStartTime-input'
                ,type: 'datetime'
            });
            laydate.render({
                elem: '#geliEndTime-input'
                ,type: 'datetime'
            });
    }
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#geliStartTime-input").val(ruleForm.geliStartTime);
        $("#geliEndTime-input").val(ruleForm.geliEndTime);
        $("#geliAddress").val(ruleForm.geliAddress);
        $("#geliContent").val(ruleForm.geliContent);
