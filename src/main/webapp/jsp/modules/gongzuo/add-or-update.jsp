...
<script>
    ...
    function setSelectOption() {
        ...
        if(gongzuoTypesSelect != null && gongzuoTypesOptions != null  && gongzuoTypesOptions.length > 0 ) {
            for (var i = 0; i < gongzuoTypesOptions.length; i++) {
                if (gongzuoTypesOptions[i].codeIndex == ruleForm.gongzuoTypes) {//下拉框value对比,如果一致就赋值汉字
                        gongzuoTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function getDetails() {
        ...
        if (addgongzuo != null && addgongzuo != "" && addgongzuo != "null") {
            //注册表单验证
            $(validform());
            $("#gongzuoDanhaoNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            $('#submitBtn').text('新增');
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $('#quyu').attr('readonly', 'readonly');
            var gongzuoContentEditor = UE.getEditor('gongzuoContentEditor');
            gongzuoContentEditor.ready(function () {
                gongzuoContentEditor.setDisabled('fullscreen');
            });
            $('#gongzuoTypesSelect').attr('readonly', 'readonly');
            $('#gongzuoTypesSelect').attr('style', 'pointer-events: none;');
        }
        else{
            // alert("未知情况.......");
        }
