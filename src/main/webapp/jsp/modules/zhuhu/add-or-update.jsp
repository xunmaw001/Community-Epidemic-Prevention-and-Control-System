            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑住户</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>入住时间</label>
                                        <input id="zhuhuTime-input" name="zhuhuTime" type="text" class="form-control layui-input">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("zhuhu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzhuhu');
    ...
    function setSelectOption() {
        ...
        if(zhuhuYimiaoTypesSelect != null && zhuhuYimiaoTypesOptions != null  && zhuhuYimiaoTypesOptions.length > 0 ) {
            for (var i = 0; i < zhuhuYimiaoTypesOptions.length; i++) {
                if (zhuhuYimiaoTypesOptions[i].codeIndex == ruleForm.zhuhuYimiaoTypes) {//下拉框value对比,如果一致就赋值汉字
                        zhuhuYimiaoTypesSelect.options[i].selected = true;
                }
            }
        }
        ...
        if(zhuhuHesuanTypesSelect != null && zhuhuHesuanTypesOptions != null  && zhuhuHesuanTypesOptions.length > 0 ) {
            for (var i = 0; i < zhuhuHesuanTypesOptions.length; i++) {
                if (zhuhuHesuanTypesOptions[i].codeIndex == ruleForm.zhuhuHesuanTypes) {//下拉框value对比,如果一致就赋值汉字
                        zhuhuHesuanTypesSelect.options[i].selected = true;
                }
            }
        }
        ...
        if(zhuhuTypesSelect != null && zhuhuTypesOptions != null  && zhuhuTypesOptions.length > 0 ) {
            for (var i = 0; i < zhuhuTypesOptions.length; i++) {
                if (zhuhuTypesOptions[i].codeIndex == ruleForm.zhuhuTypes) {//下拉框value对比,如果一致就赋值汉字
                        zhuhuTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.zhuhuContent != null && ruleForm.zhuhuContent != 'null' && ruleForm.zhuhuContent != '' && $("#zhuhuContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#zhuhuContentupload").length>0) {
    ...
    // 获取当前详情
    function getDetails() {
        var addzhuhu = window.sessionStorage.getItem("addzhuhu");
        if (addzhuhu != null && addzhuhu != "" && addzhuhu != "null") {
            //注册表单验证
            $(validform());
            $("#zhuhuDanhaoNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zhuhu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function dateTimePick() {
        if (window.sessionStorage.getItem('role') != '住户') {
            laydate.render({
                elem: '#zhuhuTime-input'
                ,type: 'datetime'
            });
        }
            laydate.render({
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#zhuhuTime-input").val(ruleForm.zhuhuTime);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $('#zhuhuDanhaoNumber').attr('readonly', 'readonly');
            $('#zhuhuLaiyuandi').attr('readonly', 'readonly');
            $('#zhuhuYimiaoTypesSelect').attr('readonly', 'readonly');
            $('#zhuhuYimiaoTypesSelect').attr('style', 'pointer-events: none;');
            $('#zhuhuHesuanTypesSelect').attr('readonly', 'readonly');
            $('#zhuhuHesuanTypesSelect').attr('style', 'pointer-events: none;');
            $('#zhuhuXingcheng').attr('readonly', 'readonly');
            var zhuhuContentEditor = UE.getEditor('zhuhuContentEditor');
            zhuhuContentEditor.ready(function () {
                zhuhuContentEditor.setDisabled('fullscreen');
            });
            debugger
            $('#zhuhuTypesSelect').attr('readonly', 'readonly');
            $('#zhuhuTypesSelect').attr('style', 'pointer-events: none;');
            $('#zhuhuTime-input').attr('readonly', 'readonly');
            // $('#zhuhuTime-input').removeAttr("aria-required");
            // $('#zhuhuTime-input').removeAttr("aria-invalid");
            // $('#zhuhuTime-input').removeAttr("lay-key");
            // $('#zhuhuTime-input').removeClass("valid");
            // $('#zhuhuTime-input').removeClass("layui-input");
        }
