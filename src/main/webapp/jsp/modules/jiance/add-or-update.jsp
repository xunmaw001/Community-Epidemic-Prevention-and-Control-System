    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑核酸检测</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">核酸检测管理</li>
                        <li class="breadcrumb-item active">编辑核酸检测</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">核酸检测信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="gongzuoId" name="gongzuoId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>检测结果</label>
                                        <select id="jianceTypesSelect" name="jianceTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>备注</label>
                                        <input id="jianceContentupload" name="file" type="file">
                                        <script id="jianceContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var jianceContentUe = UE.getEditor('jianceContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="jianceContent" id="jianceContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#jianceContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('jianceContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jiance/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjiance');
    ...
    // 查询列表
        <!-- 查询当前表的所有列表 -->
        function jianceTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiance_types", "GET", {}, (res) => {
                if(res.code == 0){
                    jianceTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationJiancetypesSelect(){
            var jianceTypesSelect = document.getElementById('jianceTypesSelect');
            if(jianceTypesSelect != null && jianceTypesOptions != null  && jianceTypesOptions.length > 0 ){
                for (var i = 0; i < jianceTypesOptions.length; i++) {
                    jianceTypesSelect.add(new Option(jianceTypesOptions[i].indexName,jianceTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(jianceTypesSelect != null && jianceTypesOptions != null  && jianceTypesOptions.length > 0 ) {
            for (var i = 0; i < jianceTypesOptions.length; i++) {
                if (jianceTypesOptions[i].codeIndex == ruleForm.jianceTypes) {//下拉框value对比,如果一致就赋值汉字
                        jianceTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.jianceContent != null && ruleForm.jianceContent != 'null' && ruleForm.jianceContent != '' && $("#jianceContentupload").length>0) {

            var jianceContentUeditor = UE.getEditor('jianceContentEditor');
            jianceContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.jianceContent != null){
                    mes = ''+ ruleForm.jianceContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                jianceContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#jianceContentupload").length>0) {
            var jianceContentEditor = UE.getEditor('jianceContentEditor');
            if (jianceContentEditor.hasContents()) {
                $('#jianceContent-input').attr('value', jianceContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                gongzuoId: "required",
                insertTime: "required",
                jianceTypes: "required",
                jianceContent: "required",
            },
            messages: {
                zhuhuId: "住户不能为空",
                gongzuoId: "工作人员不能为空",
                insertTime: "检测时间不能为空",
                jianceTypes: "检测结果不能为空",
                jianceContent: "备注不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addjiance = window.sessionStorage.getItem("addjiance");
        if (addjiance != null && addjiance != "" && addjiance != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("jiance/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#jianceContent").val(ruleForm.jianceContent);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationJiancetypesSelect();
            <!--  初始化级联表的下拉框  -->
