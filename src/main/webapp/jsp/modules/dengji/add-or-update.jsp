    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑出入登记</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑出入登记</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">出入登记信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="zhuhuId" name="zhuhuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>从哪来</label>
                                        <input id="laiAddress" name="laiAddress" class="form-control"
                                               placeholder="从哪来">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>到哪去</label>
                                        <input id="quAddress" name="quAddress" class="form-control"
                                               placeholder="到哪去">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>备注</label>
                                        <input id="dengjiContentupload" name="file" type="file">
                                        <script id="dengjiContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var dengjiContentUe = UE.getEditor('dengjiContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="dengjiContent" id="dengjiContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#dengjiContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('dengjiContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("dengji/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('adddengji');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.dengjiContent != null && ruleForm.dengjiContent != 'null' && ruleForm.dengjiContent != '' && $("#dengjiContentupload").length>0) {

            var dengjiContentUeditor = UE.getEditor('dengjiContentEditor');
            dengjiContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.dengjiContent != null){
                    mes = ''+ ruleForm.dengjiContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                dengjiContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#dengjiContentupload").length>0) {
            var dengjiContentEditor = UE.getEditor('dengjiContentEditor');
            if (dengjiContentEditor.hasContents()) {
                $('#dengjiContent-input').attr('value', dengjiContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                // laiAddress: "required",
                // quAddress: "required",
                dengjiContent: "required",
                insertTime: "required",
            },
            messages: {
                zhuhuId: "住户不能为空",
                dengjiTypes: "出入类型不能为空",
                tiwen: "体温不能为空",
                // laiAddress: "从哪来不能为空",
                // quAddress: "到哪去不能为空",
                dengjiContent: "备注不能为空",
                insertTime: "登记时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var adddengji = window.sessionStorage.getItem("adddengji");
        if (adddengji != null && adddengji != "" && adddengji != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("dengji/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#laiAddress").val(ruleForm.laiAddress);
        $("#quAddress").val(ruleForm.quAddress);
        $("#dengjiContent").val(ruleForm.dengjiContent);
